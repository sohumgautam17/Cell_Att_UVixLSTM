...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_ch, unit_ksize, unit_ch, unit_count, stride=1, padding=0):
        ...
        for idx in range(unit_count):
            unit_layer = [
                ...
                (
                    "conv2/pad",
                    nn.ReplicationPad2d((unit_ksize[1] // 2, unit_ksize[1] // 2,
                                         unit_ksize[1] // 2, unit_ksize[1] // 2)),
                ),
                (
                    "conv2",
                    nn.Conv2d(
    ...
    def forward(self, prev_feat, freeze=False):
        ...
        for idx in range(len(self.units)):
            ...
            # Resize the shortcut to match new_feat if dimensions are different
            if new_feat.size() != shortcut.size():
                shortcut = F.interpolate(shortcut, size=new_feat.size()[2:], mode='bilinear', align_corners=False)
...
class UpSample2x(nn.Module):
    ...
    def forward(self, x, target_shape):
        print(f"UpSample2x input shape: {x.shape}")
        ...
        ret = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)

        print(f"UpSample2x output shape: {ret.shape}")

        if target_shape is not None:
            print(f"Target shape for interpolation: {target_shape}")
            target_height, target_width = target_shape[-2], target_shape[-1]
            ret = F.interpolate(ret, size=(target_height, target_width), mode='bilinear', align_corners=False)
